    ...
    if (0 == xmlStrcmp (cur->name, (const xmlChar *) "action")) {
      ...
      if(is_script) {
        ...
        fprintf(fp, 
                ...
                name);
        fprintf(fp, 
                "action = gtk_action_new(\"%s\",_(\"%s\"),/* xgettext:no-c-format*/_(\"%s\"), ", 
                g_strescape((const gchar *)name, "\\"), 
                g_strescape((const gchar *)label, "\\"), 
                g_strescape((const gchar *)tooltip, "\\"));	    
        fprintf(fp, 
                "get_icon_for_name(\"%s\", \"%s\"));\n", 
                g_strescape((const gchar *)name, "\\"), 
                g_strescape((const gchar *)label, "\\"));
        if(after) 
          fprintf(fp, 
                  "g_object_set_data(G_OBJECT(action), \"after\", (gpointer)\"%s\");\n", 
        ...
        fprintf(fp, 
                "create_scheme_function_for_script(\"%s\");\n", 
                g_strescape((const gchar *)name, "\\"));
        ...
        if(new_command){
          fprintf(fp, 
                  "g_object_set_data(G_OBJECT(action), \"scheme\", \"%s\");\n", 
                  g_strescape((const gchar *) scheme, "\\"));
          if(menupath)
            fprintf(fp, 
                    "g_object_set_data(G_OBJECT(action), \"menupath\", \"%s\");\n", 
                    g_strescape((const gchar *) menupath, "\\"));
          fprintf(fp, 
