...
Some tests are automatically done here.
 ...
 - If a ```.scm``` file exists, it will be opened and the scheme will be executed on a blank score and saved. If denemo file with the same name exists (e.g. ```foobar.scm``` and ```foobar.denemo```), it will be compared to the saved file.
