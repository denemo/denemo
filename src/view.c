...
#include "scheme.h"
  ...
  install_scm_function (DENEMO_SCHEME_PREFIX"HideMenus",  scheme_hide_menus);
...
static void save_scheme_text_as(GtkWidget *widget, gchar **pfilename) {
...
static void save_scheme_text(GtkWidget *widget, gchar **pfilename) {	  
  GtkTextBuffer *buffer;
  if(*pfilename == NULL)
    save_scheme_text_as(NULL, pfilename);
  else {
    gchar *text = getSchemeText();
...
gboolean save_scheme_dialog(GtkTextBuffer *buffer, gchar **pfilename) {
  GtkWidget *dialog;
  GtkWidget *label;
  gint response;
  ...
  if(gtk_text_buffer_get_modified(buffer)) {
	  ...
	  if(response == GTK_RESPONSE_YES)
		  save_scheme_text(NULL, pfilename);
...
static void load_scheme_from_file(GtkWidget *widget, gchar **pfilename) {
  ...
  if(!save_scheme_dialog(buffer, pfilename)) {
	  return;
  }
...
void clear_scheme_window(GtkWidget *widget, gchar **pfilename) {
...
static void create_scheme_window(void) {
  ...
  Denemo.ScriptView = GTK_WIDGET(gtk_source_view_new_with_buffer(GTK_SOURCE_BUFFER(buffer)));
  gtk_source_view_set_show_line_numbers(Denemo.ScriptView, TRUE);
  gtk_source_view_set_auto_indent(Denemo.ScriptView, TRUE);
  gtk_source_view_set_indent_on_tab(Denemo.ScriptView, TRUE);
  gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(Denemo.ScriptView), GTK_WRAP_CHAR);
  ...
  g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(clear_scheme_window), &filename);

  item = gtk_menu_item_new_with_label("Open");
  gtk_menu_append(GTK_MENU(menu), item);
  g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(load_scheme_from_file), &filename);

  item = gtk_menu_item_new_with_label("Save");
  gtk_menu_append(GTK_MENU(menu), item);
  g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(save_scheme_text), &filename);

  item = gtk_menu_item_new_with_label("Save as...");
  gtk_menu_append(GTK_MENU(menu), item);
  g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(save_scheme_text_as), &filename);
