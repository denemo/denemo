 ...
 gint scale_before=*itp->scale;
  ...
  while ( (!itp->line_end) 
	 && itp->measurenum <= nummeasures)
    {
	  ...
	  (int)(
#ifdef _USE_GTK3_
  gtk_widget_get_allocated_width(Denemo.scorearea)/gui->si->zoom 
#else
  Denemo.scorearea->allocation.width/gui->si->zoom
#endif
- (RIGHT_MARGIN + KEY_MARGIN + si->maxkeywidth + SPACE_FOR_TIME)))
	...
	if((Denemo.gui->view!=DENEMO_PAGE_VIEW && itp->line_end && itp->measurenum > si->rightmeasurenum)
	  ||(Denemo.gui->view==DENEMO_PAGE_VIEW && itp->line_end && itp->curmeasure->next))
		*itp->scale = (int)
#ifdef _USE_GTK3_
  (100*x/(gtk_widget_get_allocated_width(Denemo.scorearea)/gui->si->zoom));	
#else
  (100*x/(Denemo.scorearea->allocation.width/gui->si->zoom));
#endif 
	else
	  *itp->scale = 100;
...
#define SYSTEM_SEP (6)
  ...
  gtk_widget_get_allocated_width(Denemo.scorearea) / Denemo.gui->si->zoom, SYSTEM_SEP);
#else
  cairo_rectangle (cr, 0, position-SYSTEM_SEP/2, 
  Denemo.scorearea->allocation.width/Denemo.gui->si->zoom, SYSTEM_SEP);
#endif
...
{//g_print("draw_score %p\n", cr);
  ...
  for (itp.staffnum = si->top_staff,
    ...
    while(((itp.left-gui->lefts)<DENEMO_MAX_SYSTEMS-1) && itp.line_end && (yy<(
#ifdef _USE_GTK3_
  gtk_widget_get_allocated_height(Denemo.scorearea)
#else
...
/gui->si->zoom))) {
	...
	cairo_rectangle (cr, 0, line_height-10, 
#ifdef _USE_GTK3_
  gtk_get_allocated_width(Denemo.scorearea)
#else
  Denemo.scorearea->allocation.width
#endif	
  /Denemo.gui->si->zoom, 10);
	cairo_fill(cr);
	cairo_restore(cr);
      }
  ...
  Denemo.scorearea->allocation.width
#endif
*(1-flip)*0.5/Denemo.gui->si->zoom, 0.0);	
