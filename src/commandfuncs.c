		 ...
		 gboolean rest)
{
  ...
  if (gui->mode&(INPUTRHYTHM)) {
    if(rest)
      play_pitch(64.0*(1+duration), 60.0/(4.0*gui->si->tempo*(1<<duration)), 0.1, 2);//FIXME make a distinct noise
      else
	play_pitch(64.0*(1+duration), 60.0/(gui->si->tempo*(1<<duration)), 0.2, 1);
  } else {
