      ...
      if (0 == xmlStrcmp (cur->name, (const xmlChar *) "action"))
        {
          if (cur->xmlChildrenNode == NULL)
          ...
          else
            {
              ...
              // We allow multiple menupaths for a given action, all are added to
              // the gtk_ui when this command is processed after the tooltip node.
              // This is very bad xml, as the action should have all the others
              // as children, and not depend on the order.FIXME
              menupaths = NULL; 
            }
        }
      ...
      else if (0 == xmlStrcmp (cur->name, (const xmlChar *) "tooltip"))
        {
          ...
          create_command (name, label, scheme, tooltip, hidden, after, menupath, fallback, merge, menupaths);
        }
    }
      ...
      else if (0 == xmlStrcmp (cur->name, (const xmlChar *) "bind"))
        {
          ...
          if (cur->xmlChildrenNode == NULL)
          ...
          else
            {
              ...
              if (name && tmp)
                {
                  create_binding(tmp, the_keymap, command_number);
                  xmlFree (tmp);
                }
            }
        }
      ...
      for (ncur = cur->xmlChildrenNode; ncur; ncur = ncur->next)
        {
          if ((0 == xmlStrcmp (ncur->name, (const xmlChar *) "row")))
            {
              i ? parseBindings (doc, ncur, the_keymap) 
                : parseScripts (doc, ncur, menupath, merge);
            }
          else if (i && (0 == xmlStrcmp (ncur->name, (const xmlChar *) "cursors")))
            {
...
//_!!! create a DEV version here, saving C-code to create the actions at runtime with translatable tooltips.
{
  ...
  for (i = 0; i < keymap_size (the_keymap); i++)
    {
      ...
      g_print ("%s \n", name);
#endif
      xmlNewTextChild (child, NULL, (xmlChar *) "action", (xmlChar *) name);
      if (after)
        xmlNewTextChild (child, NULL, (xmlChar *) "after", (xmlChar *) after);
      ...
      if (menupath)
        xmlNewTextChild (child, NULL, (xmlChar *) "menupath", (xmlChar *) menupath);
...
parseMenu (xmlNodePtr rootElem, gchar * path, DenemoGUI * gui)
{
  for (rootElem = rootElem->xmlChildrenNode; rootElem; rootElem = rootElem->next)
    {
      if (0 == xmlStrcmp (rootElem->name, (const xmlChar *) "menubar") || 0 == xmlStrcmp (rootElem->name, (const xmlChar *) "menu") || 0 == xmlStrcmp (rootElem->name, (const xmlChar *) "menubar") || 0 == xmlStrcmp (rootElem->name, (const xmlChar *) "toolbar"))
              ...
              if (widget)
                {
                  ...
                  // we do this in menu_click when needed create_dir_for_menu(str);
                  //FIXME we only need do this once for a given denemoui.xml
...
save_script_as_xml (gchar * filename, gchar * myname, gchar * myscheme, gchar * mylabel, gchar * mytooltip, gchar * after)
{
  ...
  xmlNewTextChild (child, NULL, (xmlChar *) "action",   (xmlChar *) myname);
