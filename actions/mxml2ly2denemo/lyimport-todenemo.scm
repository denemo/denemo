...
;;;FIXME the NEWCONTEXT can also appear at the top level, leading to repeated code. Also we are not looking to see what the new context is - just assuming Staff. and additional x_SEQUENTIAL etc
	   ...
	   ;;;((eqv? (car current_object) 'x_REALCHORD)       (begin (format #t "realchord for ~a~%" (list-tail (cdr current_object) 0))   (string-join (map loop-through (list-tail (cdr current_object) 0)))))
	...
	(begin
	  ...
	  (cond
	   ...
	   ((eqv? (car current_object) 'x_TIME) (begin (format #t "time is ~a~%~%" (cdr current_object)) (string-append "(d-InsertTimeSig \"" (cdr current_object) "\")")))
	   ((eqv? (car current_object) 'x_KEY) (begin (format #t "time is ~a~%~%" (cdr current_object)) (string-append "(d-InsertKey \"" (cadr current_object) " " (cddr current_object)  "\")")))

	   ((eqv? (car current_object) 'x_REALCHORD) (begin (format #t "hoping to process the chord for ~a~%" (caadr current_object)) (string-append (start-chord (caaadr current_object))  (string-join (map add-notes-to-chord (list-tail   (caadr current_object) 1))))))
