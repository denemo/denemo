...
(define (Abstraction::CreateMusObj)
	(define (getCurrentOffset musobj)
		(list-ref (musobj.measure musobj) (list-ref (musobj.staff musobj) OffsetTickList))) ; inner list refs gets the staff as list from OffsetTickList, outer list-ref the measure
...
;;4 check if all start-ticks are the same, if not duplicate notes to fill the gaps
	(define OffsetTickList (CreateOffsetTickListMovement))
	(define movement #f)
