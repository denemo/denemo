...
(let ((tag "ChordChartTimeSignature")(params ChordChartTimeSignature::params) 
        ...
        (let ((scale (/ (string->number (d-ScoreProperties "query=fontsize")) 7.2)))
            ...
            (d-DirectivePut-standalone-prefix tag "\\once \\override Score.RehearsalMark #'extra-offset = #'(-3 . -4)")
            (d-DirectivePut-standalone-postfix tag (string-append "\\mark \\markup {\\with-dimensions #'(-1 . 1) #'(-1 . 1) \\line\\large{\\scale #'(1.5 . 1.8) \\column{\\line\\large{\\bold "numerator"}\\line\\large \\raise #2 {\\bold "denominator"}}}}\n"))
            (d-DirectivePut-standalone-data tag "(list (cons 'x-offset \"-3\") (cons 'y-offset \"-4\"))")
