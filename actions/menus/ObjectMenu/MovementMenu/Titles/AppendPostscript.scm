...
(let ((tag "AppendPostscript")(filename #f)(width #f)(space-above #f)(space-left #f)(params AppendPostscript::params))
    ...
    (if (not filename)
        (let ((name (d-GetFilename))) (disp "Got " name " ok\n\n" DENEMO_HOME_DIR "\n\n")
            (if name
    ...
    (if (and filename width space-above space-left)
        (begin
                (d-DirectivePut-movementcontrol-postfix tag
                        (string-append "\\markup { \\hspace #" (scale space-left) " \\vspace #" (scale space-above) " \\with-url #'\"scheme:(d-AppendPostscript 'refresh)\" \\epsfile #X #" (scale width) " #\"" filename "\" }"))
