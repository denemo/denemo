...
(let ((big 60)(small 40))
    ...
    (define (NextNoteInMeasure)
        ...
        (if (Tupletmarker?)
            (skip-tuplets))
        (and (not (equal? start (d-GetHorizontalPosition))) (Music?) (not (Rest?))))

    (define (swing)
       (if (and (equal? (d-GetNoteDuration) "8") (not (d-IsGrace)) (not (d-IsTied)) (not (Rest?))
                (NextNoteInMeasure) 
                (equal? (d-GetNoteDuration) "8")  (not (d-IsGrace)) (not (d-IsTied)) (not (Rest?)))
        (begin 
            (d-MoveCursorLeft)
            (d-SetDurationInTicks big)
            (d-DirectivePut-chord-override "Duration" (logior DENEMO_OVERRIDE_GRAPHIC DENEMO_ALT_OVERRIDE))
            (d-DirectivePut-chord-prefix "Duration" "8")
            (d-MoveCursorRight)
